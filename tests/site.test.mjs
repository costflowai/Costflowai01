...
test('Base template includes feedback widget hooks', async () => {
  const html = await readFile('templates/base.html', 'utf8');
  assert.match(html, /data-feedback-panel/);
  assert.match(html, /data-feedback-form/);
});

test('Feedback widget posts to Netlify function', async () => {
  const script = await readFile('assets/js/core/feedback.js', 'utf8');
  assert.match(script, /\.netlify\/functions\/feedback/);
});
